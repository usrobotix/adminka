...
if ($current_user['role_id'] == 3 || $current_user['role_id'] == 2) { // Администратор или Модератор
    $stmt = $pdo->query("SELECT u.id, u.username, r.name AS role, u.is_active FROM users u JOIN roles r ON u.role_id = r.id");
            ...
            <tbody>
                <?php foreach ($users as $user): ?>
                    <tr>
                        ...
                        <td><?php echo $user['is_active'] ? 'Активен' : 'Деактивирован'; ?></td>
                        <?php if ($current_user['role_id'] == 3): // Только для администратора ?>
                            <td>
